...
3. Use tuple command specs; pass each command as its own positional argument, e.g. `file_exhash(path, (addr1, "d"), (addr2, "s", pat, repl))`. Use raw triple-quoted Python strings for address, pattern, replacement, and inserted text when composing commands.
...
Important:
Do not pass raw commands to Python APIs. Do not create addresses by text search or remembered line numbers, and never construct them by computing hashes (e.g. via `line_hash`): addresses come only from a fresh view immediately before the edit. On stale hash, re-view and rebuild. Where rgapi is installed, hits from `rg(pattern, lnhashs=True)` count as fresh views too: their addresses drop straight into commands (and `nbrg` finds the cell ids that `lnhashview_cell` takes). If reaching an address seems to need arithmetic, hand-copying, or a guessed hash, a step on that route was skipped. Tuple text fields can contain newlines wherever the command accepts text. For example, `(addr, "s", "foo", "bar\nbaz")` replaces one line with two. Text fields are taken verbatim: a two-character `\n` sequence stays literal; use an actual newline when you want a line break. For `a`/`i`/`c`, put all text in one tuple field: `"first\nsecond"` starts with `first`, while `"\nfirst"` inserts a leading blank line before `first`. For moving/copying between files or cells, use the qualified `m`/`t` addresses shown above. Missing files can only be created through `(r"0|0000|", "a", text)` or `(r"0|0000|", "i", text)` creation semantics.

The `%%exhash` cell magic:
In IPython sessions, importing this module registers the `%%exhash` cell magic: `%%exhash <path> [<cell_id>] <address> <a|i|c>` applies one command whose text is everything below the magic line, taken verbatim (one trailing newline stripped). Passing `<cell_id>` targets that cell in an .ipynb file instead of a plain file (`cell_exhash`); the magic dispatches on token count, so no separate cell magic exists. The line is shlex-split: quote a path containing spaces (`"My File.md"`) or escape them (`My\ File.md`). Because the text is never parsed as Python, no quoting or escaping applies. Use it for EVERY `a`/`i`/`c` command, however simple the text looks: create a file with `%%exhash path 0|0000| a`; replace a whole cell or file with `%%exhash <path> [<cell_id>] % c` (`%` needs no hashes: a full replace has no neighboring lines to mis-hit); replace a region within one with a range address and `c` (`%%exhash <path> 12|a3f2|,15|b1c3| c`), both addresses straight from the one pre-edit view. Tuple `a`/`i`/`c` text fields are only for contexts without magics (scripts, tests): interactively they add a Python quoting layer whose failure modes are not reliably foreseeable, so do not use them.
IPython expands `{expr}` and `$var` in the magic line from the user namespace (its standard `var_expand` for all magics), so a path or cell id held in a variable needs no retyping: `%%exhash {path} {cid} % c`. Only the line expands; the text stays verbatim.

Document outlines (`open_doc`):
The outline layer makes exhash a hierarchical editor: `open_doc` opens a file (`fname=`, or a `Path` as `src`; recorded for `refresh()` and edits), a URL (an `https?://` str, fetched), or any other str as text in hand, and returns a `Section` tree. Markdown sections come from headings; code files (py, js, ts, tsx, rs, zig, swift) from tree-sitter definitions; `.ipynb` files from md-heading cells over cells. Display the root bare: fixed-width rows `token title (count) [size] preview` (code rows have no title: the def line, signature included, opens the preview), previews ¶-joined with links rendered `[text][n]` so no URL spends tokens. The token is the verified address: `1.2.|12|a3f2|,45|b1c3|` fuses the dotted addr (trailing dot; the root's is `.`) with the section's boundary lnhash pair, so one row serves navigation, editing, and orientation: `at()` verifies the first hash, and the pair drops straight into a `file_exhash` range command. `at()` accepts ONLY tokens copied from a listing (`'1.6.|12|a3f2|'` or the full range form); bare dotted addrs raise, teaching the idiom. Live traversal is free: `d[1][6]`, `find(title)`, `search(pat)` (counts and matching-line previews), `paths(depth)`, `links(pat)`, and `open(n)` opens link `n` as a new tree with `base` recorded (a non-md target comes back as a leaf whose `.src` holds its text). `view()` is the reading view: rendered text with links `[text][n]` (`.src` is the raw source), displayed verbatim bare; `view(*tokens)` reads several sections in one call, each under a `# token` header when more than one, so a listing's tokens replace a print loop; `nums=`/`lnhashs=` switch any view to stored lines with edit-ready addresses. Notebook section tokens carry the heading cell id (`1.2.|ab12cd34|8f3a|`), and `view(lnhashs=True)` rows are `cellid:lineno|hash|`, ready for `cell_exhash`. The whole llms.txt docs workflow: `toc = open_doc(url)`; `toc.links(topic)`; `page = toc.open(n)`; display `page` bare; `page.view()` whole when small, else `page.search(topic)` and `page.view(*tokens)`.
"""
