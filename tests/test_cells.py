...
from exhash import lnhash, lnhashview_cell, exhash_cell
...
def mk_nb(path, cells):
    ...
    nb = dict(nbformat=4, nbformat_minor=5, metadata={},
              cells=[dict(cell_type='code', id=i, metadata={}, execution_count=None, outputs=[], source=s)
                     for i,s in cells])
...
def test_exhash_cell_inplace_list_source(tmp_path):
    p = tmp_path/'t.ipynb'
    mk_nb(p, [('aaaa1111', ['def f():\n', '    return 1'])])
    diff = exhash_cell(p, 'aaaa1111', [f"{lnhash(2, '    return 1')}s/1/2/"], inplace=True)
...
def test_exhash_cell_inplace_str_source(tmp_path):
    ...
    exhash_cell(p, 'aaaa1111', [f"{lnhash(1, 'x=1')}d"], inplace=True)
    src = json.loads(p.read_text())['cells'][0]['source']
...
def test_exhash_cell_preview_and_stale(tmp_path):
    p = tmp_path/'t.ipynb'
    mk_nb(p, [('aaaa1111', 'x=1')])
    res = exhash_cell(p, 'aaaa1111', [f"{lnhash(1, 'x=1')}s/1/9/"])   # no inplace
