...
impl From<exhash_core::EditResult> for EditResultPy {
    fn from(r: exhash_core::EditResult) -> Self {
        Self { lines: r.lines, hashes: r.hashes, modified: r.modified, deleted: r.deleted }
    }
}

#[pyfunction]
fn line_hash(line: &str) -> String { format!("{:04x}", exhash_core::line_hash_u16(line)) }

#[pyfunction]
fn lnhash(lineno: usize, line: &str) -> String { exhash_core::format_lnhash(lineno, line) }

#[pyfunction]
fn lnhashview(text: &str) -> Vec<String> {
    text.lines()
        .enumerate()
        .map(|(i, l)| format!("{}  {}", exhash_core::format_lnhash(i + 1, l), l))
        .collect()
}
...
fn py_exhash(text: &str, cmds: Vec<String>) -> PyResult<EditResultPy> {
    ...
    let parsed = exhash_core::parse_commands_from_strs(&cmd_refs)
        .map_err(|e| PyValueError::new_err(e.to_string()))?;
    let res = exhash_core::edit_text(text, &parsed)
        .map_err(|e| PyValueError::new_err(e.to_string()))?;
    Ok(res.into())
}
