...
pub enum Subcommand {
    ...
    Global { invert: bool, pattern: String, cmd: Box<Subcommand> },
    Indent { levels: usize },
    Dedent { levels: usize },
    Sort,
    Print,
    ...
    F: FnMut() -> Result<Vec<String>, EditError>,
{
    ...
    if rest.is_empty() { return Err(EditError::new("missing command")); }

    let (cmd, trailing) = parse_subcommand_with_text(rest, &mut read_text)?;

    // No trailing junk for a top-level command.
    if !trailing.trim().is_empty() { return Err(EditError::new(format!("unexpected trailing characters: {:?}", trailing))); }
...
pub fn command_from_parts(addr: &str, cmd: Subcommand) -> Result<Command, EditError> {
    ...
    if !rest.trim().is_empty() { return Err(EditError::new(format!("unexpected trailing characters in address: {:?}", rest))); }
    build_command(addr1, addr2, has_comma, cmd)
}
...
pub fn parse_destination_address(input: &str, op: char) -> Result<Address, EditError> { parse_destination_address_inner(input, op, false) }

pub(crate) fn parse_buffer_destination_address(input: &str, op: char) -> Result<Address, EditError> { parse_destination_address_inner(input, op, true) }
    ...
    F: FnMut() -> Result<Vec<String>, EditError>,
{
    ...
    if let Some(trailing) = s.strip_prefix("sort") { return Ok((Subcommand::Sort, trailing)); }

    // g! must be checked before g
    if let Some(rest) = s.strip_prefix("g!") { return parse_global(rest, true, read_text); }
    ...
    F: FnMut() -> Result<Vec<String>, EditError>,
{
    ...
    if delim.is_alphanumeric() || delim == '\\' { return Err(EditError::new("global delimiter must not be alphanumeric or backslash")); }
    let (pat, after_pat) = parse_delimited(rest, delim)?;
    let cmd_str = after_pat.trim_start();
    if cmd_str.is_empty() { return Err(EditError::new("global requires a subcommand")); }
    let (subcmd, trailing) = parse_subcommand_with_text(cmd_str, read_text)?;
    if !trailing.trim().is_empty() { return Err(EditError::new(format!("unexpected trailing characters in global subcommand: {:?}", trailing))); }
...
pub fn subst_from_parts(pattern: String, replacement: String, flags: &str) -> Result<Subst, EditError> {
    ...
    for ch in flags.trim().chars() {
        match ch { 'g' => global = true, 'i' => case_insensitive = true, _ => return Err(EditError::new(format!("unknown substitute flag: {ch}"))) }
    }

    if pattern.is_empty() { return Err(EditError::new("substitute pattern may not be empty")); }
    ...
    loop {
        match it.next() {
            ...
            Some(line) => {
                ...
                if line == "." { break; }
                if line == ".." { out.push(".".to_string()); } else { out.push(line.to_string()); }
            }
        }
    }
    Ok(out)
